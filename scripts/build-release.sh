...
# Flint Release Build Script
# Builds, signs, notarizes, and packages the flint binary for macOS
#
# Prerequisites:
...
#   ./scripts/build-release.sh --op --skip-notarize
...
banner() {
    echo ""
    echo -e "${BLUE}+==========================================+${NC}"
    echo -e "${BLUE}|       Flint Release Build                |${NC}"
    echo -e "${BLUE}+==========================================+${NC}"
    echo ""
}
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                ...
                echo "  --skip-notarize    Skip notarization (binary still signed)"
                echo "  --skip-build       Reuse existing binary in target/"
...
create_tarball() {
    ...
    local version=$(get_version)
    local tar_name="${BINARY}-${version}-darwin-arm64.tar.gz"

    cd "$DIST_DIR"
    tar -czf "$tar_name" "$BINARY"
    shasum -a 256 "$tar_name" > "${tar_name}.sha256"
    log_info "Created: $tar_name"
}
...
verify_artifacts() {
    ...
    echo ""
    echo "=== Version ==="
    "$DIST_DIR/$BINARY" --version 2>/dev/null || true
}
...
main() {
    ...
    if [[ "$SKIP_PKG" == true ]]; then
        ...
        if [[ -x "$MUNKIPKG" ]]; then
            ...
            log_warn "munkipkg not found — skipping PKG (install with: pip3 install munkipkg)"
        fi
    fi
